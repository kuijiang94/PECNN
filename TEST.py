...
def weight_loader(model_weights=None, tfvars=None):
    ...
    for v in tfvars:
        ...
        if (layer_name in model_weights) and (param_name in model_weights[layer_name]):
            print('initializing: ', layer_name, '\t', param_name,
                  model_weights[layer_name][param_name].shape)

            # if 'conv'
            loaders.append(
                tf.assign(v, model_weights[layer_name][param_name], name='as_' + layer_name))
    return tf.group(*loaders)

def self_ensemble(img):
    imgco=[]
    imgco.append(img)
    imgco.append(img.transpose(Image.FLIP_LEFT_RIGHT))
    imgco.append(img.transpose(Image.ROTATE_180))
    imgco.append(imgco[1].transpose(Image.ROTATE_180))
    imgco.append(img.transpose(Image.ROTATE_90))
    imgco.append(imgco[4].transpose(Image.FLIP_LEFT_RIGHT))
    imgco.append(imgco[4].transpose(Image.ROTATE_180))
    imgco.append(imgco[5].transpose(Image.ROTATE_180))
    return imgco
...
class VIDEOSR(object):
        ...
        for i in range(num_frame):
            ...
            with tf.variable_scope('srmodel', reuse=reuse) as scope_sr:#prelu
                with slim.arg_scope([slim.conv2d], activation_fn=tf.nn.relu, stride=1,
                    ...
                    res_in1 = conv3
                    reuse_block=False
                    for p in range(3):#11.5个64x64x3x3 加上SE块P=10
    ...
    def test(self, path=None, scale_factor=3, num_frames=1):
        ...
        for pat in file:
            if file.index(pat)>=startid:
                ...
                for idx0 in range(len(inList)):
                    ...
                    if len(dims) == 3 and idx0==self.num_frames//2:#tif
